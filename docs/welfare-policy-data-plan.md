...
**완료**: 스키마 반영 + 크롤러 파이프라인 1차 구현·검증
- 공공데이터포털 API는 점검 중이라 크롤링으로 선회
- `app/intake/collector/` — 목록 fetch(`html_fetcher.py`) → 키워드 필터 → 상세 fetch → LLM 구조화 추출(`llm_extractor.py`) → 저장(`pipeline.py`)
- 대상: 서울특별시(1) + 강남구·강동구·영등포구(3, 시드 데이터 지역 기준) + 서민금융진흥원
- 실사이트(강남구/강남구보건소, 이지웹 계열 CMS) 기준 end-to-end 동작 검증 완료. LLM이 무관한 행정공고를 정확히 걸러내는 것까지 확인
- **미해결**: 강동구·영등포구·서민금융진흥원·서울시(워드프레스 계열)는 게시판 CMS 구조가 달라 상세글 링크 판별(`_looks_like_detail_link`)이 안 맞음 → 아직 저장된 실제 정책 0건

### 다음 단계
- 위 4개 사이트에 맞는 링크 판별 로직 보강 (사이트별 오버라이드 또는 더 범용적인 휴리스틱)
- 공공데이터포털 점검 끝나면 API 연동도 병행 (더 안정적인 1차 소스)
- 지자체 공고 크롤러 범위 확대 (17개 광역 + 실증 기초지자체 5~10곳)
- 보건소/자치법규정보시스템 연동
- 중복 제거·정규화 로직
